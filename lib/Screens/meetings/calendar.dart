  ...
  void _handleEventClicked(CalendarEventData<Object?> event) {
    ...
    if (id.startsWith("E_")) {
      route = MaterialPageRoute(
        builder: (context) => EventDetailsPage(eventId: id.replaceAll("E_", "")),
      );
    } else {
      route = MaterialPageRoute(
        builder: (context) => MeetingDetailsPage(meetingId: id.replaceAll("M_", "")),
      );
    }
        ...
        body: StreamBuilder<QuerySnapshot>(
            ...
            builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> events) {
              return StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> meetings) {
                    ...
                    if (meetings.hasData && meetings.data != null) {
                      eventController.addAll(meetings.data!.docs
                          .where((meeting) => meeting.get("startDate") != null && meeting.get("endDate") != null)
                          .map((meeting) => CalendarEventData(
                              event: "M_${meeting.id}",
                              title: meeting.get("name"),
                              description: meeting.get("description"),
                              startTime: (meeting.get("startDate") as Timestamp).toDate(),
                              endTime: (meeting.get("endDate") as Timestamp).toDate(),
                              date: (meeting.get("startDate") as Timestamp).toDate(),
                              color: const Color(0xFFB3B2B1),
                              endDate: (meeting.get("endDate") as Timestamp).toDate()))
                          .toList());
                    }
                    ...
                    return Column(children: [
                      ...
                      if (currentView == "Woche")
                        Expanded(
                            child: WeekView(
                                controller: eventController, onEventTap: (e, d) => _handleEventClicked(e.first))),
                      if (currentView == "Tag")
                        Expanded(
                            child: DayView(
                                controller: eventController, onEventTap: (e, d) => _handleEventClicked(e.first)))
